...
class DragGesturePullToRefresh extends VerticalDragGestureRecognizer {
  ...
  // Add a callback for network error
  ...
  /// [callback] Function to call when network is unavailable
  DragGesturePullToRefresh setNetworkErrorCallback(Function callback) {
  ...
  /// start refresh with network check
  Future<void> refresh() async {
    // Check for internet connection before refreshing
    bool hasInternet = await ConnectivityService().checkInternetConnectionV();

    if (!hasInternet) {
      // Call the network error callback if available
      if (onNetworkError != null) {
        onNetworkError!();
      }

      // Complete the completer to hide the refresh indicator
    ...
    // Continue with normal refresh if internet is available
    if (!completer.isCompleted) {
      completer.complete();
    }
    completer = Completer<void>();
