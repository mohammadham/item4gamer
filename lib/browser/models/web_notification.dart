...
abstract class WebNotificationPermissionDb {
  ...
  static Future<Map<String, String>> getPermissions() async {
    final prefs = await _prefs;
    return prefs
        .getKeys()
        .where((key) => key.startsWith('notification_permission_'))
        .fold<Map<String, String>>({}, (map, key) {
      map[key.replaceFirst('notification_permission_', '')] =
          prefs.getString(key)!;
      return map.isEmpty ? Map.from(_db) : map;
    });
  }

  static Future<String?> getPermissionString(String host) async {
    final prefs = await _prefs;
    return prefs.getString('notification_permission_$host');
  }

  static Future<void> savePermissionString(
      String host, String permission) async {
    final prefs = await _prefs;
    await prefs.setString('notification_permission_$host', permission);
  }
}
