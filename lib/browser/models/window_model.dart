...
class WindowModel extends ChangeNotifier {
  ...
  void addTab(WebViewTab webViewTab) {
    if (_webViewTabs.any((t) => t.key == webViewTab.key)) {
      throw Exception("Tab with duplicate key detected!");
    }
  ...
  Future<void> flushInfo() async {
    ...
    if (window == null || window.isEmpty) {
      count = await db?.rawInsert(
          'INSERT OR REPLACE INTO windows(id, json) VALUES(?, ?)',
  ...
  void hideAllTabs() {
    _webViewTabs.forEach((tab) => tab.webViewModel.isHidden = true);
    notifyListeners();
  }

  void restoreHiddenTabs() {
    _webViewTabs.forEach((tab) => tab.webViewModel.isHidden = false);
    notifyListeners();
  }
}
